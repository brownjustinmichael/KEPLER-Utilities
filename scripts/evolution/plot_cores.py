...
if namespace.checkcache:
    ...
    cache (session, q.add_entity (SimulationEntry).filter (DumpFileEntry.state == "presn").all (), {cache: getattr (kepler_utils.database.cache, cache) for cache in caches}, states = [state for state in namespace.states])
